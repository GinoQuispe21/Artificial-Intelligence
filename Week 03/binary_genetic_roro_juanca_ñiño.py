...
def tourn_select(indiv_list):
    ...
    new_indiv_list = []
    for i in range(0, n_indiv, 2):
        if(indiv_list[posis[i]].x > indiv_list[posis[i+1]].x):
...
def cruce(new_indiv_list):
    ...
    for i in range(0, n_indiv, 2):
        ...
        x1 = int(temp1,2)
        x2 = int(temp2,2)
        indiv1 = Individual(i+1, x1,x1**2, temp1)
        indiv2 = Individual(i+2, x2,x2**2, temp2)
...
def mutation(new_indiv_list):
    ...
    for indiv in new_indiv_list:
        ...
        if(choice == 1):
            ...
            indiv = Individual(i+1, int(temp,2),int(temp,2)**2 , temp)
